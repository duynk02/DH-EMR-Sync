...
const ExecuteJSMain = (async () => {
  ...
  try {
    ...
    v.expressENVBase64 = v.RequestJson?.body?.expressENVBase64 || "";
    v.pgserviceENVBase64 = v.RequestJson?.body?.pgserviceENVBase64 || "";
    function normalizeBase64(str) {
      return str + "=".repeat((4 - (str.length % 4)) % 4);
    }
    if (v.expressENVBase64 !== "" && fs.existsSync(v.expressENVPath) === true) {
      console.log(`Cập nhật expressENVBase64: ${v.expressENVPath}`);
      fs.writeFileSync(v.expressENVPath, Buffer.from(normalizeBase64(v.expressENVBase64), "base64").toString("utf8"), "utf8");
    }
    if (v.pgserviceENVBase64 !== "" && fs.existsSync(v.pgserviceENVPath) === true) {
      console.log(`Cập nhật pgserviceENVBase64: ${v.pgserviceENVPath}`);
      fs.writeFileSync(v.pgserviceENVPath, Buffer.from(normalizeBase64(v.pgserviceENVBase64), "base64").toString("utf8"), "utf8");
    }
